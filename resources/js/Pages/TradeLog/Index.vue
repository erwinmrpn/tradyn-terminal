...
const props = defineProps<{
    ...
    accounts: any[];
    totalBalance: number;
    selectedAccountId: string; // ID dari controller (bisa 'all' atau angka)
}>();

// --- FILTERED ACCOUNTS ---
// Memisahkan akun Spot dan Futures agar dropdown bersih
const filteredAccounts = computed(() => {
    if (!props.accounts) return [];
    return props.accounts.filter(acc => acc.strategy_type === props.activeType);
});

// --- SMART DEFAULT LOGIC (FIX BUG BLANK) ---
// Fungsi ini bertugas memilih akun pertama secara otomatis
const applySmartDefaults = () => {
    // Pastikan ada akun di list
    if (filteredAccounts.value.length > 0) {
        const firstAccountID = filteredAccounts.value[0].id;

        // 1. Set Form Default (Wajib akun pertama, jangan kosong)
        form.trading_account_id = firstAccountID;

        // 2. Set Filter Default (Jika current selection 'all' atau tidak valid di tab ini)
        // Kita cek apakah akun yang sedang dipilih user ada di list tab yang aktif?
        const isCurrentSelectionValid = filteredAccounts.value.some(acc => acc.id === selectedAccount.value);
        
        // Jika User sedang pilih 'All' atau pilih akun Futures padahal lagi di tab Spot -> PAKSA GANTI ke Akun Pertama
...
onMounted(() => {
    ...
    // [FIX] Jalankan smart default saat halaman pertama kali dimuat
    applySmartDefaults();
});
...
// --- LOGIC FILTER ---
// Kita inisialisasi dengan props, tapi nanti bisa berubah oleh Smart Defaults
const selectedAccount = ref(props.selectedAccountId);

const switchTab = (type: string) => {
    // Saat ganti tab, kita reset filter ke 'all' dulu biar logic Smart Default bekerja di Watcher props
    router.get(route('trade.log'), { type: type, account_id: 'all' }, { preserveState: true, preserveScroll: true });
};

// Watcher untuk Filter Global (Reload data saat ganti akun)
watch(selectedAccount, (newAccount) => {
    // Hanya reload jika nilai valid dan beda dengan yang ada di URL sekarang
...
// Watcher saat Props berubah (misal habis ganti tab Spot <-> Futures)
watch(() => props.activeType, () => {
    ...
    // [FIX] Jalankan Smart Default setiap kali Tab berubah
    // Kita pakai nextTick agar computed filteredAccounts update dulu
    nextTick(() => {
        applySmartDefaults();
    });
});
...
// --- FORM INIT ---
const form = useForm({
    trading_account_id: '', // Nanti diisi otomatis oleh applySmartDefaults
...
const submitTrade = () => {
    ...
    form.post(route('trade.log.store'), {
        ...
        onSuccess: () => {
            ...
            // [FIX] Pastikan akun default tetap terpilih setelah submit
            applySmartDefaults(); 
        },
        onError: (errors) => {
