...
// Fungsi Ganti Tab (Mengganti URL parameter ?type=...)
const switchTab = (type: string) => {
    router.get(route('trade.log'), { type: type }, {
        preserveState: true,
        preserveScroll: true,
    });
};
                ...
                <div class="flex justify-center mb-8">
                    <div class="bg-[#1a1b20] p-1.5 rounded-full flex items-center w-full max-w-sm border border-[#2d2f36] relative shadow-inner">
                        ...
                        <button 
                            ...
                            class="flex-1 py-2 rounded-full text-sm font-bold transition-all duration-300 ease-out z-10 relative"
                            :class="props.activeType === 'SPOT' 
                        ...
                        <button 
                            ...
                            class="flex-1 py-2 rounded-full text-sm font-bold transition-all duration-300 ease-out z-10 relative"
                            :class="props.activeType === 'FUTURES' 
                                ? 'text-white shadow-lg' 
                                : 'text-gray-500 hover:text-gray-300'"
                        >
                            FUTURES
                        ...
                        <div 
                            class="absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-emerald-500 rounded-full transition-all duration-300 ease-out shadow-[0_0_15px_rgba(16,185,129,0.4)]"
                            :class="props.activeType === 'SPOT' ? 'left-1.5' : 'left-[calc(50%+3px)]'"
                        ></div>
                            ...
                            <tbody class="divide-y divide-[#1f2128]">
                                <tr v-for="trade in props.trades" :key="trade.id" class="hover:bg-[#1a1b20]/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="px-2 py-0.5 text-xs font-bold rounded uppercase"
                                            :class="trade.type === 'LONG' ? 'text-green-400 bg-green-900/20' : 'text-red-400 bg-red-900/20'">
                                            {{ trade.type }}
                                        </span>
                                    </td>

                                    <td class="px-6 py-4 text-right text-gray-300 font-mono text-sm">
                                        {{ trade.entry_price }}
                                    </td>
                                    <td class="px-6 py-4 text-right text-gray-300 font-mono text-sm">
                                        {{ trade.exit_price || '-' }}
                                    ...
                                    <td class="px-6 py-4 text-right font-bold"
                                        :class="Number(trade.pnl) >= 0 ? 'text-green-500' : 'text-red-500'">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded border"
                                            :class="trade.status === 'CLOSED' 
                                                ...
                                                : 'bg-yellow-900/30 text-yellow-400 border-yellow-500/20 animate-pulse'">
                                            {{ trade.status }}
                                        </span>
                                    </td>
                                </tr>
                                ...
                                <tr v-if="props.trades.length === 0">
                                    <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                        No {{ props.activeType.toLowerCase() }} trades recorded yet.
                                    </td>
                                </tr>
                            </tbody>
                        </table>
